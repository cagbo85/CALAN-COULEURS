...
@section('content')
        ...
        @if ($editions->isEmpty())
            <div class="max-w-2xl mx-auto">
                <article class="overflow-hidden border-2 shadow-xl bg-white/95 backdrop-blur-sm rounded-xl border-white/50">
                    <div class="p-8 text-white" style="background: linear-gradient(to right, #272AC7, #8F1E98);">
                    ...
                    <div class="p-8 text-center">
                        ...
                        <div class="flex justify-center gap-2 mb-2">
                            <div class="w-2 h-2 bg-[#272AC7] rounded-full animate-bounce"></div>
                ...
                <div class="mb-8 border-b border-gray-200">
                    <nav class="flex flex-wrap justify-center space-x-1 sm:space-x-8" role="tablist">
                        @foreach ($editions as $edition)
                            <button
                                class="gallery-tab px-4 py-3 text-sm font-medium border-b-2 {{ $loop->first ? 'border-[#272AC7] text-[#272AC7] hover:text-[#8F1E98] hover:border-gray-300' : 'border-transparent text-gray-500 hover:text-[#8F1E98] hover:border-gray-300' }}"
                            ...
                            <div class="gap-5 columns-2 sm:columns-3 lg:columns-4">
                                @foreach ($galleryByYear[$edition->year] as $image)
                                    <div class="mb-5 break-inside-avoid">
                                        <button
                                            class="w-full block rounded-lg overflow-hidden shadow-sm hover:shadow-md focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#272AC7]"
    ...
    <div id="imageModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black/85" role="dialog"
        ...
        <button
            class="absolute left-4 top-1/2 -translate-y-1/2 bg-[#272AC7]/30 border-none text-white text-6xl leading-none px-4 py-2 rounded-lg cursor-pointer hover:bg-[#272AC7]/50 select-none"
        ...
        <button
            class="absolute right-4 top-1/2 -translate-y-1/2 bg-[#272AC7]/30 border-none text-white text-6xl leading-none px-4 py-2 rounded-lg cursor-pointer hover:bg-[#272AC7]/50 select-none"
            ...
            document.querySelectorAll(".gallery-tab").forEach(btn => {
                btn.addEventListener("click", function() {
                    document.querySelectorAll(".gallery-tab").forEach(b => {
                        b.classList.remove("border-[#272AC7]", "text-[#272AC7]");
                    ...
                    this.classList.add("border-[#272AC7]", "text-[#272AC7]");
